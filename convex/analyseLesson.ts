...
import { layoutConceptGraph, validateDiagnosticQuestions } from "./lib/education";
...
export const conceptGraph = actionGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const graph = layoutConceptGraph(response.output_parsed);
...
export const diagnostic = actionGeneric({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (!response.output_parsed) throw new Error("model_refusal_or_empty_output");
      const questions = validateDiagnosticQuestions(response.output_parsed.questions, context.graph);
