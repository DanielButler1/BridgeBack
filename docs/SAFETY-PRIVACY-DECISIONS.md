...
flowchart LR
    ...
    P -->|"Optional activity-grounded voice"| R["OpenAI Realtime API"]
    R -->|"Speech response over WebRTC"| P
```
...
### AI-007: Keep voice support short, contextual and optional

**Decision:** “Talk it through” uses `gpt-realtime-2.1` over WebRTC for an optional five-minute conversation about the current teacher-approved activity. The session receives only the activity title, objective, explanation and worked example. It does not receive the pupil's name, attendance, reason for absence, diagnostic answers or wider school record. BridgeBack stores session model, topic, start time, status and duration limit for quota and audit purposes, but does not store audio or a transcript.

**Why:** Voice can make learning more accessible to pupils who understand by explaining and questioning aloud. Keeping the session attached to one activity reduces disclosure and educational drift. WebRTC is OpenAI's recommended browser transport, and the permanent API key remains server-side. A stable one-way user hash is bound as the Realtime safety identifier.

**Controls:** The interface identifies the voice as AI, asks the pupil not to share private information, provides mute and end controls, and closes the browser connection after five minutes. Server-side reservation limits each account to six sessions in a rolling 24-hour period. The tutor prompt prohibits grading, personal-data collection and investigation of safeguarding disclosures, and directs serious personal concerns to a trusted adult. The five-minute client closure is a product control, not a cryptographic server-enforced media cutoff, so production cost and duration monitoring remain required.

The active Realtime model is selected by the server-only `OPENAI_REALTIME_MODEL` setting. The browser is model-agnostic, allowing a future publicly documented voice model to be evaluated and adopted without changing the pupil interface or exposing model selection to the client.

**Real-school limitation:** No real child voice may be processed until the under-18 release blockers in this document are satisfied, including age-appropriate disclosure, moderation and reporting, safeguarding escalation, DPIA approval, and appropriate OpenAI retention controls. OpenAI specifically requires Zero Data Retention before processing personal data of children below 13 or the applicable age of digital consent.
